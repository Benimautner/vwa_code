"""
source: https://github.com/Ceyron/machine-learning-and-simulation/blob/main/english/simulation_scripts/lid_driven_cavity_python_simple.py
...
def main():
    ...
    def laplace(f):
        diff = np.zeros_like(f)
        diff[1:-1, 1:-1] = (f[1:-1, 0:-2] + f[0:-2, 1:-1] - 4 * f[1:-1, 1:-1] + f[1:-1, 2:] + f[2:, 1:-1]) / (
                element_length ** 2)
        return diff
    ...
    def set_boundary_conditions(u, v):
        ...
        u[0, :] = 0#HORIZONTAL_VELOCITY_TOP # bottom
        u[:, 0] = 0.0  # Left
        u[:, -1] = 0.0
        u[-1, :] = HORIZONTAL_VELOCITY_TOP  # Top
        v[0, :] = 0.0
        v[:, 0] = 0.0
        v[:, -1] = 0.0
        v[-1, :] = 0 #HORIZONTAL_VELOCITY_TOP
    ...
    for _ in tqdm(range(N_ITERATIONS)):
        ...
        u_tent = (u_prev + TIME_STEP_LENGTH * (
               -(u_prev * d_u_prev__d_x + v_prev * d_u_prev__d_y) + KINEMATIC_VISCOSITY * laplace__u_prev))
        v_tent = (v_prev + TIME_STEP_LENGTH * (
                -(u_prev * d_v_prev__d_x + v_prev * d_v_prev__d_y) + KINEMATIC_VISCOSITY * laplace__v_prev))
        #u_tent = (u_prev + TIME_STEP_LENGTH * (
        #        -(u_prev * d_u_prev__d_x + v_prev * d_u_prev__d_y)))
        #v_tent = (v_prev + TIME_STEP_LENGTH * (
        #        -(u_prev * d_v_prev__d_x + v_prev * d_v_prev__d_y)))

        set_boundary_conditions(u_tent, v_tent)
